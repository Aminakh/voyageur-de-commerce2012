...
public boolean detecteCroisement(ArrayList<Integer> l, int[][] coordville, int i, int j){
	...
	double a1 = (coordville[l.get(i+1)][2]-coordville[l.get(i)][2]) / (coordville[l.get(i+1)][1]-coordville[l.get(i)][1]);
	double b1 = coordville[l.get(i)][2] - (a1*coordville[l.get(i)][1]);
	
	if (coordville[l.get(j+1)][1] != coordville[l.get(j)][1]){
		//calcul du coefficient directeur de la deuxieme droite
	double a2 = (coordville[l.get(j+1)][2]-coordville[l.get(j)][2]) / (coordville[l.get(j+1)][1]-coordville[l.get(j)][1]);
	double b2 = coordville[l.get(j)][2] - (a2*coordville[l.get(j)][1]);
...
public boolean appartientFigure(ArrayList<Integer> l, int i, int j, int [][] coordville, int x,int y){
	boolean bool = false;
	...
	if (x >= coordville[l.get(i)][1] & x <= coordville[l.get(i+1)][1]  & x >= coordville[l.get(j)][1] & x <= coordville[l.get(j+1)][1]){
	//if (y > coordville[l.get(i)][2] & y < coordville[l.get(i+1)][2]/*  & y > coordville[l.get(j)][2] & y < coordville[l.get(j+1)][2]*/){
